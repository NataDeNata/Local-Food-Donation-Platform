...
export class DonationFormComponent implements AfterViewInit {
  ...
  model: Omit<Donation, 'id' | 'status' | 'postedAt'> & { freshness?: string; expiryDate?: string } = {
    title: '',
    description: '',
    quantity: 1,
    location: '',
  ...
  async submit(form: NgForm) {
    if (form.invalid || !this.model.contactName || !this.model.contactPhone) {
      alert('Please fill out all required fields, including contact name and phone.');
      return;
    }

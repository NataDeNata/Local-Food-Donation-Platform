...
export class AuthService {
  ...
  firestoreUser = signal<any | null>(null);
    ...
    onAuthStateChanged(this.auth, async (user) => {
      ...
      if (
        ...
        if (snap.exists()) {
          ...
          this.firestoreUser.set(data);
        } else {
          await setDoc(userDoc, { email: user.email, role: 'admin' });
          this.userRole.set('admin');
          this.firestoreUser.set({ email: user.email, role: 'admin' });
        }
        return;
      }
      ...
      if (user) {
        ...
        if (snap.exists()) {
          ...
          this.firestoreUser.set({ email: user.email, role: 'user' });
        }
      } else {
        this.userRole.set(null);
        this.firestoreUser.set(null);
  ...
  /**
   * Assigns or updates the barangay admin role for a user in Firestore.
   * @param uid User ID
   * @param barangayName Name of the barangay to assign
   */
  async setBarangayAdmin(uid: string, barangayName: string) {
    const userDoc = doc(this.firestore, 'users', uid);
    await updateDoc(userDoc, {
      role: 'admin',
      barangayAdmin: barangayName
    });
  }
}
